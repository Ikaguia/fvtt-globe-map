...
(async () => {
    ...
    const symlinkPath = path.resolve(dataPath, "modules", "fvtt-globe-map");
    ...
    if (symlinkStats) {
        ...
        const proceed = (
            await prompts({
                ...
                name: "value",
                initial: false,
                message: `A "fvtt-globe-map" ${atPath} already exists in the "modules" subfolder. Replace with new symlink?`,
            })
        ).value;
        if (!proceed) {
